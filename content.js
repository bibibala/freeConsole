...
(function () {
    ...
    if (header) {
      ...
      function dragEnd() {
        ...
        if (rect.left < snapThreshold) {
          ...
          finalLeft = windowWidth - rect.width - 10; // 吸附到右边缘，留10px边距
        }
        ...
        // 确保不超出视窗
        finalLeft = Math.max(10, Math.min(finalLeft, windowWidth - rect.width - 10));
        finalTop = Math.max(10, Math.min(finalTop, windowHeight - rect.height - 10));
      ...
      function adjustPositionOnResize() {
        ...
        let newLeft = parseFloat(panel.style.left) || (windowWidth - rect.width - 20);
        let newTop = parseFloat(panel.style.top) || (windowHeight - rect.height - 20);
        ...
        chrome.storage.local.set({panelPosition: position}, () => {
          console.log('窗口大小调整后位置已保存:', position);
        });
      }
  ...
  function addLog(message, level = 'log') {
    ...
    if (isDuplicate) {
      // console.log('检测到重复日志，跳过:', message);
      return;
    }
